...
test("isLlamaRuntimeBundleComplete requires impl dll on Windows", () => {
  ...
  writeFileSync(join(dir, "llama-server.exe"), "stub");
...
test("copyLlamaRuntimeBundle copies all files from extract dir", async () => {
  ...
  writeFileSync(join(extractDir, "llama-server.exe"), "exe");
  ...
  assert.equal(isLlamaRuntimeBundleComplete(installDir), process.platform !== "win32");
  rmSync(extractDir, { recursive: true, force: true });
  rmSync(installDir, { recursive: true, force: true });
});
